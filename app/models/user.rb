class User < ApplicationRecord
  ...
  has_many :inverted_friendrequests, -> { where status: false }, class_name: 'Friendrequest', foreign_key: 'friend_id'
  has_many :friend_requests, through: :inverted_friendrequests, source: :user

  def confirm_friend(user)
    friend = Friendrequest.find_by(user_id: user.id, friend_id: id)
    friend.status = true
    friend.save
